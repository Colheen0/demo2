...
export default function List() {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      if (!listId) return;
      try {
        // Route sécurisée : le token est dans le header
        ...
        if (response.ok && data.tasks) {
          setTasks(data.tasks.map((t: any) => ({ id: t._id, name: t.name, completer: t.completer })));
            ...
            console.log("Erreur chargement tâches:", response.data);
        }
      } catch (e) {
        Alert.alert("Erreur", "Impossible de charger les tâches", e as any);
      }
    };
    fetchTasks();
  }, [listId]);
  ...
  const handleToggleTask = async (id: string, completer: boolean) => {
    try {
        // Backend vérifie le token + id de la tâche
  ...
  const handleUpdateTask = async (id: string, newName: string) => {
    try {
        ...
        Alert.alert("Erreur", "Renommage impossible");
    }
  };
...
const styles = StyleSheet.create({
  ...
  scrollView: { flex: 1 },
  header: { flexDirection: "row", alignItems: "center", paddingHorizontal: 16, paddingVertical: 16, backgroundColor: "#fff", borderBottomWidth: 1, borderBottomColor: "#e0e6ed" },
  ...
  progressBar: { height: 6, backgroundColor: "#e0e6ed", borderRadius: 3, overflow: "hidden" },
  ...
  fab: { position: "absolute", bottom: 20, right: 20, width: 56, height: 56, borderRadius: 28, backgroundColor: "#3498db", justifyContent: "center", alignItems: "center", shadowColor: "#3498db", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.4, shadowRadius: 12, elevation: 8 },
});